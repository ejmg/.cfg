    ...
    #+BEGIN_SRC emacs-lisp
      (use-package lsp-mode
        :hook ((rust-mode . lsp)
               (web-mode . lsp)
               (elm-mode . lsp)
               ;; TODO: Figure out if either of these really are worth it
               ;; (lua-mode . lsp)
               ;; (python-mode . lsp)
...
**** TODO spotify
     ...
     a very clean and functional client for spotify that needs to still be configured for usage if i
     want it. i use ncspotify for the most part now, but if the async support has improved then i'd
     probably be convinced into switching as i could then run spotify using spotifyd >:)
     #+BEGIN_SRC emacs-lisp
       (use-package spotify
         :load-path "~/.emacs.d/site-lisp/spotify.el")
...
**** vue(?)
     ...
     #+END_SRC
*** elm
    i need to set this up properly for elm as i have for rust mode with respect to lsp mode.
    #+begin_src emacs-lisp
    ;; TODO
    ;; only valid when using elm-mode, should configure correctly as such
    ...
    #+begin_src emacs-lisp
      (use-package lsp-lua-emmy
        ...
        (setq lsp-lua-emmy-jar-path
              (expand-file-name "EmmyLua-LS-all.jar"
                                (concat default-directory "lsp-lua-emmy"))))
