...
**** TODO org
     ...
     #+BEGIN_SRC emacs-lisp
       (defun generic-org-minor-modes ()
           ...
           ;; allow body text to indent to its headline's level via <tab>
...
***** org pomodoro
      ...
      #+BEGIN_SRC emacs-lisp
        (use-package org-pomodoro
          :init
          (setq org-pomodoro-length 35) ;; i need more time to get into a groove
          (setq org-pomodoro-long-break-frequency 3) ;; 4 is too long imo
...
****** TODO clock
       ...
       session of a task. When I want that, set this to ='current=. When i want only for today, that's
       ='today=. for total time, ='auto= or ='all= works.
       #+BEGIN_SRC emacs-lisp
...
****** Displaying the clock
       where to display the clocked time for the current work item. i can do =both= to have it on both
       frame and mode-line, or one of =mode-line= and =frame-title=. the problem with setting it to
       =frame-title= is that it seems to steal focus from my window manager which makes tabbing
       through applications confusing when it changes every minute.
...
**** anki mode
     ...
     #+END_SRC
**** all the icons
     there are packages (such as lsp mode and it's minor modes, treemacs, etc) that use fancy
     icons. not having them installed leads to missing character boxes like you would expect for
     utf8. might as well just install them. NOTE: must run =all-the-icons-install-fonts= for this to
     actually go to use. 
     #+begin_src emacs-lisp
     ;; TODO probably should config to run `all-the-icons-install-fonts` as post install hook command
       (use-package all-the-icons)
     #+end_src
**** treemacs
     provides a nice project directory outline
     #+begin_src emacs-lisp
     (use-package treemacs)
     #+end_src
**** pomidor
     a simple pomidoro mode that provides a nice graphical counter/tracker. while over complicating
     things, i sometimes like to use it alongside org's clocking mechanism in place of org-pomidoro,
     particularly when i'm doing something that isn't computer-focused, ie taking notes from a book

     #+begin_src emacs-lisp
       (use-package pomidor
         :config
         ; 35 minute pomidoro
         (setq pomidor-seconds (* 60 35)))
     #+end_src
** TODO keybindings
...
**** TODO local leader
     - [ ] move local leader declaration above into this subsection
     - [ ] is there a better way to clear, preserve the top-level "global" local-leader than
       preemptively rebinding them to nil????
       + ie what happens if two local-mode bindings end up using the same leading key? will that
         break things? yes? no?


     For some reason unbinding a leading key stroke to chain more complex commands, eg like:
     #+begin_src emacs-lisp :tangle no
       (local-leader
         :keymap 'org-mode-map
         "t" '(:ignore t :which-key "description")
         "ti" 'madeup-function-with-mneumonic-i)
     #+end_src

     then the top-level of key =local-leader= becomes polluted irrespective of the mode with
     keystroke =local-leader t= showing ="t -> +description"=. the work around i have for this now is
     simply preemptively binding any leading keys used by a local mode like this to nil. feels
     like i'm either defining these keystrokes wrong and/or there is a better way of binding
     leading keys for a "local leader" "in the first place.
...
*** TODO org mode bindings
    ...
    #+BEGIN_SRC emacs-lisp
      (local-leader
        ...
        "ll" 'org-insert-link
        "lc" 'counsel-org-link
        "e" '(:ignore t :which-key "exports")
        "ep" 'org-latex-export-to-pdf
        "el" 'org-latex-export-as-latex
        "eh" 'org-html-export-as-html
        "em" 'org-md-export-as-markdown
        "eo" 'org-odt-export-to-odt)
    #+END_SRC
...
*** rust keybindings
    ...
    #+END_SRC
*** ocaml keybindings
    same story as rust bindings
    #+begin_src emacs-lisp
      (local-leader
        :keymaps 'tuareg-mode-map
        "b" '(:ignore b :which-key "build")
        "b" #'(lambda () (interactive) (compile "dune build ."))
        "o" 'tuareg-opam-update-env
        ;; TODO? Do i actually use this? Delete?
        "h" 'hs-toggle-hiding
        "l" '(:ignore l :which-key "lsp")
        "l." 'lsp
        "lr" 'lsp-restart-workspace
        "t" #'(lambda () (interactive) (compile "dune test ."))
        "u" '(:ignore u :which-key "utop")
        "uu" 'utop
        "ur" 'utop-eval-region
        "ub" 'utop-eval-buffer
        "ur" 'utop-restart
...
*** python
    ...
    i use pyenv to manage python virtual environments. it Just Works and keeps things simple + makes
    installing python versions trivial and separate from my system's setup.
    #+begin_src emacs-lisp
      (use-package pyenv-mode
        :hook python-mode)
    #+end_src

    anaconda-mode is my favorite minor-mode for working with python and getting intelligent
    completion that doesn't involve lsp and isn't as heavy as elpy:
    #+begin_src emacs-lisp
...
*** rust
    ...
    #+begin_src emacs-lisp
      (defun rust-company-backends ()
        (setq-local company-backends
                    (mapcar #'company-mode/backend-with-yas '((company-capf company-keywords company-gtags)
...
*** TODO webprogramming
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (use-package web-mode
        ...
        (defun my-web-mode-setup ()
          ...
          (setq web-mode-markup-indent-offset 2) ; this applies to jsx
          (electric-pair-mode)
          (set-fill-column 120)
          (linum-mode 1)
      ...
      ;; TODO disable electric-pair-local for tera files
    ...
    #+end_src
    rainbow mode allows me to actually see the colors of stylings when editing scss/sass with
    web-mode:
    #+begin_src emacs-lisp
...
*** lua
    ...
    #+begin_src emacs-lisp
      (defun lua-company-backends ()
        (setq-local company-backends
                    '((company-capf company-keywords company-gtags)
                      company-dabbrev-code
                      company-files)))
...
*** c/c++
**** ccls
     here is a first attempt at getting ccls setup for emacs and lsp mode. will it work? to be
     determined.
     #+begin_src emacs-lisp
       (use-package ccls
...
*** TODO ocaml
    ...
    TODO: just switch to lsp, apparently the perf is as good if not better than merlin now.

**** TODO user-setup
     - [ ] delete this section
       + [ ] get comfortable with lsp + tuareg + utop
       + [ ] see about pulling out individual opam utility functions from =opam-user-setup.el= and
         putting them somewhere else to keep.

     a really nice library that sets up emacs to use ocaml with no effort via =opam install user-setup=
     and following the instructions. *no longer using* because of performance of =lsp= being
     on-par, if not /better/, than =merlin=. everything below was old config.
    
     provided by =opam user-install=. moved from init.el into here.
     ...
     this file sources resources provided by =user-install=, which basically configures emacs as
     required for ocaml dev that roughly aligns with how i would do it by hand. these files are all
     found in =~/.opam/default/share/emacs/site-lisp/=.
    
**** merlin

     auto-completion for ocaml before lsp. very good, but will probably be phased out. use for
     utop-mode still, however.
         ...
         (setq merlin-command
               (concat
                (file-name-as-directory
                 (substring (shell-command-to-string "opam var bin --switch=ocamlformat --safe") 0 -1))
                "ocamlmerlin")))
...
**** ocamlformat
     ...
     initially added for the cs3110 class, but i'm switching to ocamlformat and ocaml-lsp in general.

     however, to help keep my version of ocamlformat isolated from all other packages and ocaml
     installs, we use a designated switch for ocamlformat called, well, =ocamlformat=. that's what the
     lambda in the use-package declaration is doing for us. this was originally taken from the
     ocamlformat README itself, but it was seemingly removed. [[https://github.com/ocaml-ppx/ocamlformat/blob/31ce39658661ed0f8a9545071813e66f74e84798/doc/editor_setup.mld#L42-L70][you can still find it here]]. the end
     result is we have =ocamlformat= set to work on any ocaml code we open because of
     ='enable-outside-detected-project=, and we use an isolated version of ocamlformat in all
     instances.
         ...
         (lambda ()
           (concat
                ...
                (file-name-as-directory
                 ...
                 (substring (shell-command-to-string "opam var share --switch=ocamlformat --safe") 0 -1))
...
**** utop
     utop is the REPL used by most ocaml developers and it can be run inside of emacs.

     #+begin_src emacs-lisp
       (defun utop-company-backends ()
          (setq-local company-backends
        ...
        (use-package utop
          ...
          :config
          ;; run utop via dune to build and auto-load project modules.
          (setq utop-command "opam config exec -- dune utop . -- -emacs")
          (add-hook 'utop-mode-hook #'utop-company-backends))
     #+end_src

**** TODO dune
     dune is the de-facto build tool of ocaml now with the exception of how the compiler itself is
     managed.
...
**** tuareg
     ...
     tuareg is the mode used for editing ocaml along with popular ocaml DSL's like Menhir's =.mly= and
     jsoo's =.eliom= files.

     #+begin_src emacs-lisp
       (defun ocaml-company-backends ()
         (setq-local company-backends
                     (mapcar #'company-mode/backend-with-yas
                             '((company-capf company-keywords company-gtags)
                               company-dabbrev-code
       ...
       (use-package tuareg
         ...
         (add-hook 'tuareg-mode-hook #'ocaml-company-backends))
     #+end_src
